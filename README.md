...
A lightweight macOS menu bar utility that keeps your Mac awake, with a live system monitor in the same popover.
...
Live readouts (sampled only while the Stats tab is open):

- **CPU** and **GPU** utilization
- **Memory** used / total
- **Storage** used / total on the boot volume
- **Battery** percent, charging state, **health**, **cycle count**, **power draw**, and temperature
...
**Stats.** CPU comes from Mach `host_statistics`; memory from `host_statistics64`; GPU from IOAccelerator `PerformanceStatistics`; battery/power from `IOPSCopyPowerSourcesInfo` plus the `AppleSmartBattery` IORegistry entry; disk from Foundation volume resource values.
...
├── PopoverRoot.swift          Shared header, Awake / Stats switcher, panel sizing
├── ContentView.swift          Awake tab: duration, closed lid, Settings, About, Quit
...
Click **Settings** on the Awake tab. Preferences are persisted to `UserDefaults`:
